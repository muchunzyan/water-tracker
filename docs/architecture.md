...
- Напиток содержит название, процент гидратации, цвет и иконку или иллюстрацию.
...
Акцентная палитра синхронизирована с логотипом Oasis: primary `#0077b6`,
hover `#005f8f` и белый foreground `#ffffff`. Вспомогательные цвета иллюстрации — aqua
`#48cae4`, deep water `#023e8a`, leaf `#90be6d` и sand `#ffd166` — используются
в брендовых тенях, графиках и PWA-иконках. Эти значения считаются проектными
токенами поверх пресета.

Поля ввода и выпадающие списки используют единый control-height `52px`, фон карточки,
шрифт `16px` и одинаковую горизонтальную геометрию. Пункты раскрытого списка используют
тот же шрифт `16px` и минимальную высоту `48px` для комфортного чтения и нажатия.
...
`WaterTrackerDatabase`. Текущая версия схемы — `5`. Она содержит три object store:
...
erDiagram
  ...
  SETTINGS {
    ...
    boolean useTemperatureAdjustment
    object weather
    object training
  }
```
...
| `hydrationPercent` | `number`  | Процент эффективной гидратации            | целое число от −500 до 150   |
...
добавляет первый расширенный набор категорий. Миграция версии 4 заменяет неизменённые
системные определения каталогом из 116 напитков, удаляет устаревшие системные позиции
и добавляет новые. Пользовательские изменения и ранее удалённые встроенные напитки при
этом сохраняются. Миграция версии 5 удаляет устаревшее поле стандартной порции: объёмы
для новой записи определяются по истории пользователя, а при пустой истории применяется
начальное значение 250 мл. Встроенные и пользовательские напитки можно редактировать и
удалять; настройки предоставляют явное восстановление исходного встроенного каталога.
...
| `effectiveHydrationMl` | `number`        | Рассчитанный эффективный объём        | целое число от −25 000 до 7 500 |
...
| `useTemperatureAdjustment` | `boolean` | Разрешена ли погодная поправка    | по умолчанию `false`                                |
| `weather`                  | `object`  | Суточный прогноз и координаты     | необязателен; дата, °C, координаты и время загрузки |
| `training`                 | `object`  | Часы тренировок за локальный день | необязателен; дата и 0–24 часа                      |
...
`weather` является однодневным офлайн-кэшем ответа Open-Meteo. Запрос выполняется
только при включённом `useTemperatureAdjustment`, завершённом onboarding и отсутствии
прогноза на текущую локальную дату. Ошибка сети, запрет геолокации или невалидный ответ
не меняют настройки и не блокируют приложение. `training` также содержит дату, поэтому
запись предыдущего дня интерпретируется как 0 часов без отдельной фоновой миграции.
