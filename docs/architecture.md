...
- **CSS Custom Properties** — дизайн-токены, темы, цвета, размеры и отступы.
- **SVG-компоненты** — иконки, иллюстрации напитков и индикатор гидратации.
- Собственный компактный набор UI-компонентов без тяжёлой сторонней UI-библиотеки.

На первом этапе не используются Tailwind CSS и готовый UI-фреймворк. Это позволит создать выразительный собственный стиль и не привязывать дизайн к ограничениям библиотеки.

Дизайн-система реализована через глобальные семантические CSS-переменные в `src/styles/tokens.css`. Светлая и тёмная темы переключают значения токенов через атрибут `data-theme` корневого элемента. Системная тема разрешается в React-провайдере через `prefers-color-scheme`. Базовые UI-компоненты размещаются в `src/ui`, а общий адаптивный каркас — в `src/app/AppShell`.
...
`WaterTrackerDatabase`. Текущая версия схемы — `1`. Она содержит три object store:
...
| `isBuiltin`        | `boolean` | Признак встроенного напитка               | встроенные нельзя удалять    |
...
данные добавляются транзакцией `populate` только при первом создании базы.
Встроенные напитки доступны только для чтения: репозиторий запрещает их изменение и
удаление. Пользовательские напитки можно свободно редактировать и удалять.
...
отсутствует, возвращаются настройки по умолчанию: цель 2 000 мл и системная тема.
